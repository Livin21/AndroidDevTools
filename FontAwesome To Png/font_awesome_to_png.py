...
#
# fontawesome-to-android.py 
#
# Exports Font Awesome icons as PNG images in android 
#
# Copyright (c) 2012-2014 Michal Wojciechowski (http://odyniec.net/)
#
# Font Awesome - http://fortawesome.github.com/Font-Awesomei
# fontawesome-to-android.py
#

import sys, argparse, re
from os import makedirs, path, access, R_OK
...
def export_icon(icon, size, filename, font, color, margin, dirname):
    ...
    # Save file
    outimage.save(dirname + "/" + filename )
...
if __name__ == '__main__':
    ...
    parser.add_argument("--size", type=int, default=32,
            help="Icon size in pixels for mdpi (default: 32)")
    ...
    # create android bucket dirs
    for bucket in buckets:

        if not bucket in dip_scaler_map:
            print >> sys.stderr, ("Error: Invalid bucket (%s), skipping." % (bucket))
            continue
        ...
        for icon in selected_icons:
            ...
            dip_scaler = dip_scaler_map[bucket]

            export_icon(icon, int(size*dip_scaler), filename, font, color, margin, dirname)